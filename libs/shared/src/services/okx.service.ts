...
export class OKXService {
   ...
   */
  public async generateCompoundTransaction(
    chainId: number,
    ...
    if (commissionAmount > 0) {
      ...
      const commissionTx = await this.getCommissionTransaction(
        chainId,
        tokenInAddress,
        creator,
        commissionAmount,
    ...
    if (tokenInAddress !== '0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee') {
      const approveTx = await this.getApproveData(
        chainId,
        tokenInAddress,
        swapAmount,
      );
    ...
    // 3. Swap transaction
    const swapTx = await this.getSwapData(
      account,
      chainId,
      tokenInAddress,
      tokenOutAddress,
   ...
   */
  public async getCommissionTransaction(
    chainId: number,
    tokenInAddress: string,
    creator: string,
